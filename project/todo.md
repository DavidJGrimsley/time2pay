...
- [ ] We need a 'mark as paid' button in the invoices screen for time2pay (not mercury) invoices. This should just be a button that adds a 'paid' tag to it and changes the background color to green. We can also add a date paid field that is optional.
- [ ] We should add a 'notes' field to the invoice builder that is just a text field that the user can type in. This will be added to the invoice PDF and the invoice details page. This is for any additional information the user wants to include on the invoice that is not covered by the existing fields.
- [ ] We should add a 'due date' field to the invoice builder that is just a date picker. This will be added to the invoice PDF and the invoice details page. This is for the user to specify when the invoice is due.
- [ ] We should add a 'send invoice' button that allows the user to send the invoice via email. This will require the user to input the client's email address and will send a PDF copy of the invoice to that email. We can use a service like SendGrid or Mailgun for this functionality.
- [ ] We need an onboarding flow for new users that guides them through setting up their profile, connecting Mercury (if applicable) and github and other integrations, This could be a multi-step  that appears when customers sign UP and until they complete the steps. It should be like my pokepages flow (giving info about the app, then asking them to fill out their profile, then asking them to connect integrations) But I saw some cool versions online where it's scrollable and has a checklist at the top that you're kinda scrolling through. 