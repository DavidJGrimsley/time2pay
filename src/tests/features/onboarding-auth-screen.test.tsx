...
describe('OnboardingAuthScreen tour exit', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('exits tour mode and resets local mode before redirecting to /dashboard', async () => {
    ...
    const { default: OnboardingAuthScreen } = await import('@/features/onboarding/auth-screen');

    let root!: renderer.ReactTestRenderer;
    await act(async () => {
      root = renderer.create(<OnboardingAuthScreen />);
    });
  ...
  it('exits tour mode in hosted mode without touching local-mode reset', async () => {
    ...
    await act(async () => {
      renderer.create(<OnboardingAuthScreen />);
    });
